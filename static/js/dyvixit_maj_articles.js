function update_info_section(){
	...
	function(data){
		jQuery.each(data, function(){
			...
			+ '<p><a target="_blank" href="' + this.fields.link +'"><img src="/dyvixitsolutions/media/' 
...
function update_astuce_section(){
	...
	$.getJSON("/dyvixitsolutions/update-astuce-after/" + most_recent.attr('id') + "/",
	function(data){
		jQuery.each(data, function(){
...
$(document).ready(function(){
	setInterval("update_articles()", 20000); // On rafraichit le contenu toutes les minutes (60.000 millisecondes).
});